...
class Renderer:
    ...
    def _draw_title(self, game_state):
        ...
        self.safe_addstr(self.stdscr, 0, 0, t_str.center(self.term_w)[:self.term_w-1], attr)

    def _draw_status_bar(self, player, game_state):
        """Bottom status bar."""
        w = player.equipment.get("weapon")
        wname = w["name"] if w else "Unarmed"
        conds = ",".join(list(player.conditions.keys())[:4]) if player.conditions else "None"
        s = (f" [{player.cls.upper()}] {player.name} | "
    ...
    def _draw_left_sidebar(self, player, game_state):
        ...
        put(9,  f"Food {surv_bar(player.hunger)}", curses.color_pair(surv_color(player.hunger)))
        put(10, f"Watr {surv_bar(player.thirst)}", curses.color_pair(surv_color(player.thirst)))
        put(11, f"Ftg  {surv_bar(player.fatigue)}", curses.color_pair(surv_color(player.fatigue)))
        put(12, f"Warm {surv_bar(player.warmth)}", curses.color_pair(surv_color(player.warmth, invert=False)))
        put(13, f"Morl {surv_bar(player.morale)}", curses.color_pair(surv_color(player.morale, invert=False)))
        ...
        skill_names = [
            ("kenjutsu","Sword"),("kyujutsu","Bow"),("jujutsu","Grapple"),
            ("naginata","Polearm"),("stealth","Stealth"),("rhetoric","Rhetoric"),
            ("survival","Survival"),("medicine","Medicine"),("teppo","Firearms"),
        ]
        for i, (sk, short) in enumerate(skill_names):
            lv = player.skills.get(sk, 1)
            bar = "●" * lv + "○" * (10 - lv)
            col = curses.color_pair(3) if lv >= 7 else (curses.color_pair(7) if lv >= 4 else curses.color_pair(8))
    ...
    def _draw_right_sidebar(self, world, player, game_state):
        ...
        put(14, "─────────────────", curses.color_pair(8))
        put(15, f"Weight: {player.current_weight:.1f}/{player.max_carry_weight}kg", attr_lbl)
        put(16, f"Ammo: →{player.ammo.get('arrow',0)} ●{player.ammo.get('ball',0)}", attr_lbl)
        ...
        for nid, npc in world.npcs.items():
            ...
            if t and t.lit and npc.alive:
                dist = math.sqrt((npc.col-player.col)**2 + (npc.row-player.row)**2)
    ...
    def _draw_messages(self):
        ...
        y_start = self.msg_y
        # Separator
        self.safe_addstr(self.stdscr, y_start, 0,
                         "─" * (self.term_w - 1), curses.color_pair(8))
    ...
    def draw_inventory(self, player):
        ...
        lines.append("")
        lines.append(f"  Carried: {player.current_weight:.1f}/{player.max_carry_weight}kg")
    ...
    def draw_character_sheet(self, player):
        ...
        for faction, rep in player.rep.items():
            if abs(rep) > 0:
                bar = "+" * max(0, rep // 10) if rep > 0 else "-" * max(0, abs(rep) // 10)
        ...
        for sy in range(h - 2):
            for sx in range(w - 2):
                ...
                if 0 <= wc < WORLD_W and 0 <= wr < WORLD_H:
                    ...
                    td = TERRAIN[tile.terrain]
                    self.safe_addstr(self.stdscr, draw_y, draw_x, td["ch"],
        ...
        try:
            ...
            for city in CITIES[:15]:
                ...
                if 1 <= sx < w-3 and 1 <= sy < h-2:
                    short = city["name"][:4]
                    self.safe_addstr(self.stdscr, start_y + 1 + sy, start_x + 1 + sx,
                                     short, curses.color_pair(3) | curses.A_BOLD)
        except:
            pass

        self.safe_addstr(self.stdscr, start_y + h - 1, start_x + 2,
                         " Press M or Esc to close ", curses.color_pair(7))
        self.stdscr.refresh()
    ...
    def draw_dialog(self, npc, lines, topics, selected=0):
        ...
        if topics:
            ...
            for i, (key, text) in enumerate(list(topics.items())[:8]):
                ...
                dialog_lines.append(f"  {marker}[{i+1}] {key.replace('_',' ').title()}")
        dialog_lines.append("")
    ...
    def draw_rest_screen(self, hours):
        """Show rest animation."""
        self.safe_addstr(self.stdscr, self.term_h // 2, self.term_w // 2 - 10,
                         f"  Resting... {hours}h  ", curses.color_pair(6) | curses.A_BOLD)
        self.stdscr.refresh()
    ...
    def _draw_modal(self, lines, title=""):
        ...
        for i, line in enumerate(lines[:h-3]):
            self.safe_addstr(self.stdscr, start_y + 2 + i, start_x + 1,
                             line[:w-2], curses.color_pair(1))
        self.stdscr.refresh()
    ...
    def _draw_box(self, y, x, h, w, title=""):
        ...
        # Fill background
        for ry in range(h):
            self.safe_addstr(self.stdscr, y + ry, x, " " * (w-1), curses.color_pair(0))
    ...
    def prompt_string(self, prompt, max_len=20):
        ...
        self.safe_addstr(self.stdscr, y, x, prompt + " " * (max_len + 2),
    ...
    def draw_char_creation(self, selected_class, selected_region):
        ...
        self.safe_addstr(self.stdscr, y,   x, "╔══════════════════════════════════════════════════════╗", attr_title)
        self.safe_addstr(self.stdscr, y+1, x, "║          縁起 ENGI — Character Creation              ║", attr_title)
        self.safe_addstr(self.stdscr, y+2, x, "╚══════════════════════════════════════════════════════╝", attr_title)

        self.safe_addstr(self.stdscr, y+4, x, "Choose your class:  (← → to cycle)", attr_lbl)
        ...
        for i, cls_key in enumerate(class_keys):
            ...
            self.safe_addstr(self.stdscr, y+5+i, x, f"  {marker} {cls['name']}", col)
        ...
        if selected_class in CLASSES:
            ...
            self.safe_addstr(self.stdscr, y+15, x, f"  Start HP:{cdef['hp']}  Ki:{cdef['stamina']}  Honor:{cdef['start_honor']}  Money:{cdef['start_money']}mon", attr_desc)
        ...
        self.safe_addstr(self.stdscr, y+19, x, "Starting Region:  (↑ ↓ to select)", attr_lbl)
        for i, (rkey, rdesc) in enumerate(regions):
            ...
            self.safe_addstr(self.stdscr, y+20+i, x, f"  {marker} {rdesc[:70]}", col)

        self.safe_addstr(self.stdscr, y+25, x, "─" * 56, attr_lbl)
        self.safe_addstr(self.stdscr, y+26, x, "  [Enter] Begin your story  |  [Q] Quit", attr_sel)
        self.stdscr.refresh()
